...
export function generateMarkdownReport(report: ValidationReport): string {
  ...
  lines.push(`|-------------|----------|`);
  lines.push(`| Total Addressable Market (TAM) | ${report.marketInsights.tamEstimate} |`);
  lines.push(`| Serviceable Addressable Market (SAM) | ${report.marketInsights.samEstimate} |`);
  lines.push(`| Serviceable Obtainable Market (SOM) | ${report.marketInsights.somEstimate} |`);
  ...
  for (const competitor of report.competitors) {
    ...
    if (competitor.fundingStage) lines.push(`**Funding:** ${competitor.fundingStage}`);
    if (competitor.estimatedUsers) lines.push(`**Scale:** ${competitor.estimatedUsers}`);
  ...
  const medPriority = report.improvements.filter((i) => i.priority === "medium");
  ...
  lines.push(`---`);
  lines.push("");
  lines.push(`*Report generated by [App Idea Validator](/) — AI-powered startup validation*`);
